...
const getInitialProviders = ({ purest }) => ({
    async web3({ query }) {
        const { signature, message, address, timestamp } = query;

        if (!(signature && +timestamp && address && message)) {
            throw new Error('Signature, message, timestamp, address is required params.');
        }
